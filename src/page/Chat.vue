...
<template>
  <div class="bigbox">
    <van-sticky :offset-top="0.0001"
                class="headerTitile">
      <van-nav-bar :title="this.$route.params.apName">
        <template #left>
        ...
        <div class="chatUser-box"
          ...
          <div class='chatUser-box-img'>
            <van-image round
                       width="2.5rem"
                       height="2.5rem"
                       :src="current == item.senderId ? item.hrImg : item.apImg" />
          </div>
...
export default {
  ...
  data () {
    // 这里存放数据
    return {
      current: localStorage.getItem('hrid'),
      websock: null,
      msg: '',
      infoData: [
      ]
  ...
  methods: {
    initWebSocket () {
      ...
      console.log('建立连接')
      this.infoData =JSON.parse(localStorage.getItem(this.$route.params.uid+'_'+localStorage.getItem('hrid'))).data
    },
     getNowFormatDate () {
      var date = new Date()
      var seperator1 = '-'
    ...
    websocketonopen () {
      // 连接建立之后执行send方法发送数据
      var data = {
        senderId: localStorage.getItem('hrid'),
        receiverId: this.$route.params.uid,
        hrImg: localStorage.getItem('hrImg'),
        apImg: this.$route.params.apImg,
    ...
    websocketonmessage (e) {
      ...
      localStorage.setItem(this.$route.params.uid+'_'+localStorage.getItem('hrid'),JSON.stringify(form))
    },
    websocketsend (Data) {
      // 数据发送
